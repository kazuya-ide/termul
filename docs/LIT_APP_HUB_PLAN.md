...
4. **自動更新への影響**: このリポジトリ(`feat/lit-app-hub`)はupstreamの署名鍵を持たないため、ビルドしたバイナリの自動更新チェックは機能しない(先のJP化フォークと同じ制約)。個人利用ツールとして使う前提なら問題ないが、方針として明記しておく必要がある(→確認事項)。
